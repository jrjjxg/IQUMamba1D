"""DualDomainMambaDualPath — V1 + Dual-Path Mamba in Freq Tower (controlled ablation).

This is an EXACT copy of dual_domain_mamba.py (V1) with ONE change:
  - FreqConvBlock: flatten(C*F)+giant Mamba  →  Dual-Path Mamba (intra-freq + inter-time)
    * d_model = out_channels (NOT out_channels * freq_bins)
    * Intra-frame Mamba runs along frequency axis
    * Inter-frame Mamba runs along time axis

Purpose: isolate the effect of Dual-Path Mamba vs flatten Mamba in the freq tower.
All other components (bidirectional cross-attention with sigmoid(-2) gates,
time tower, decoder) are IDENTICAL to V1.
...
class DualPathMambaBlock(nn.Module):
    """Dual-path Mamba: intra-frame (freq axis) + inter-frame (time axis).

    Inspired by TF-GridNet's intra/inter BLSTM, but using Mamba (SSM)
    for O(L) complexity. d_model = C (channels only), keeping the
    frequency-axis topology intact.
    """
    ...
    def forward(self, x):
        ...
        # Intra-frame: Mamba along F axis — (B*T, F, C)
        x_f = x.permute(0, 3, 2, 1).reshape(B * T, Fr, C)
        x_f = x_f + self.mamba_intra(self.norm_intra(x_f))
        x = x_f.reshape(B, T, Fr, C).permute(0, 3, 2, 1)

        # Inter-frame: Mamba along T axis — (B*F, T, C)
        x_t = x.permute(0, 2, 3, 1).reshape(B * Fr, T, C)
        x_t = x_t + self.mamba_inter(self.norm_inter(x_t))
...
class FreqConvBlock(nn.Module):
    ...
    Compared to V1's FreqConvBlock:
      V1:  flatten(C*F) -> Mamba(d_model=C*F)   — parameter explosion
      Here: DualPathMamba(d_model=C)              — parameter efficient
    """
    ...
    def __init__(self, in_channels: int, out_channels: int,
        ...
        # CHANGED: Dual-Path Mamba (d_model = out_channels, NOT out_channels * freq_bins)
    ...
    def forward(self, x):
        ...
        # CHANGED: Dual-Path Mamba instead of flatten + giant Mamba
...
class CrossAttentionFusion(nn.Module):
    ...
    def __init__(self, time_dim: int, freq_channels: int, freq_bins: int,
        ...
        # Learnable fusion gate (init to -2 → sigmoid ≈ 0.12, conservative start)
...
class DualDomainMambaDualPath(nn.Module):
