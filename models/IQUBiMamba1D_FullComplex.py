"""Full-complex BiMamba U-Net for IQ source separation.

This variant keeps encoder, BiMamba blocks, skip fusion, decoder, and output
heads in paired real/imag complex form.  The only boundary conversions are:

  input  (B, 2, T)      -> complex tensor (B, 2, C, T)
  output (B, 2, K, T)   -> training tensor (B, 2*K, T)

The BiMamba operation is implemented as a complex wrapper around two real
BiMamba-style Mamba cores:
...
class _RealBiMambaCore(nn.Module):
    """BiMamba transform without residual, used as a real component of complex Mamba."""
...
class ComplexBiMambaLayer(nn.Module):
    """Complex-valued wrapper around two real BiMamba cores."""
    ...
    def __init__(
        ...
        super().__init__()
        self.dim = int(dim)
        self.real_core = _RealBiMambaCore(self.dim, d_state=d_state, d_conv=d_conv, expand=expand)
        self.imag_core = _RealBiMambaCore(self.dim, d_state=d_state, d_conv=d_conv, expand=expand)
        self.channel_token = bool(channel_token)

    def _complex_fuse(self, real: torch.Tensor, imag: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        yr = self.real_core(real) - self.imag_core(imag)
        yi = self.imag_core(real) + self.real_core(imag)
        return yr + real, yi + imag

    def forward_patch_token(self, x: torch.Tensor) -> torch.Tensor:
        real, imag = _split_complex(x)
        batch, channels = real.shape[:2]
        dims = real.shape[2:]
        n_tokens = dims.numel()
        real_flat = real.reshape(batch, channels, n_tokens).transpose(-1, -2)
        imag_flat = imag.reshape(batch, channels, n_tokens).transpose(-1, -2)
        yr, yi = self._complex_fuse(real_flat, imag_flat)
        yr = yr.transpose(-1, -2).reshape(batch, channels, *dims)
        yi = yi.transpose(-1, -2).reshape(batch, channels, *dims)
        return _stack_complex(yr, yi)

    def forward_channel_token(self, x: torch.Tensor) -> torch.Tensor:
        real, imag = _split_complex(x)
        batch, n_tokens = real.shape[:2]
        dims = real.shape[2:]
        real_flat = real.flatten(2)
        imag_flat = imag.flatten(2)
        yr, yi = self._complex_fuse(real_flat, imag_flat)
        return _stack_complex(yr.reshape(batch, n_tokens, *dims), yi.reshape(batch, n_tokens, *dims))
...
class ComplexResBlock1D(nn.Module):
    """Residual block whose convolutions, normalizations, and activations are complex."""
...
class ComplexStemBlock1D(nn.Module):
    """Convert raw IQ samples into complex feature maps."""
...
class ComplexUpsampleLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = _interpolate_complex(x, scale_factor=self.scale_factor)
        return self.proj(x)


class ComplexSkipConnectionProcessor(nn.Module):
    """Complex skip fusion used by the decoder."""
...
class ResidualBiMambaEncoder_FullComplex(nn.Module):
    """Complex encoder with complex-wrapped BiMamba at the same stages as IQUBiMamba1D."""
    ...
    def __init__(
        ...
        super().__init__()
        if input_channels != 2:
            raise ValueError(f"Full-complex IQ model expects input_channels=2, got {input_channels}")
...
class ComplexUNetResDecoder(nn.Module):
    """Complex decoder that emits standard concatenated IQ source channels."""

    def __init__(self, encoder: ResidualBiMambaEncoder_FullComplex, num_classes: int, n_conv_per_stage, deep_supervision):
        super().__init__()
        if num_classes % 2 != 0:
            raise ValueError(f"Full-complex decoder expects even num_classes=2*num_sources, got {num_classes}")
...
class IQUBiMamba1D_FullComplex(nn.Module):
    """BiMamba U-Net whose main feature path stays complex-valued end to end."""
    ...
    def __init__(
        ...
        super().__init__()
        self.encoder = ResidualBiMambaEncoder_FullComplex(
        ...
        self.decoder = ComplexUNetResDecoder(
