"""DualDomainMambaCrossMamba — V1 + Cross-Mamba Fusion (controlled ablation).

This is an EXACT copy of dual_domain_mamba.py (V1) with ONE change:
  - CrossAttentionFusion (bidirectional MultiheadAttention + sigmoid(-2) gate)
    → CrossMambaFusion (concat time+freq tokens → Mamba → zero-init gate)

Purpose: isolate the effect of replacing Attention with Mamba for cross-domain fusion.
All other components (flatten Mamba freq tower, time tower, decoder) are IDENTICAL to V1.
...
class CrossMambaFusion(nn.Module):
    """Cross-domain fusion via Mamba instead of Attention.

    Instead of Q/K/V attention between time and freq tokens, we:
    1. Pool freq along F axis → (B, C_f, T_f) → project to D_t
    2. Add domain embeddings to distinguish time vs freq tokens
    3. Concatenate [time_tokens; freq_tokens] → run Mamba → take time portion
    4. Zero-init gate for stable warm-up

    Complexity: O(L_t + T_f) vs Attention's O(L_t × T_f)
    """
    ...
    def __init__(self, time_dim: int, freq_channels: int, freq_bins: int,
        ...
        # Freq projection: pool F axis → project channels to time_dim
        self.freq_pool_f = nn.AdaptiveAvgPool2d((1, None))  # (B, C_f, 1, T_f)
        self.freq_proj = nn.Sequential(
            nn.Conv1d(freq_channels, time_dim, 1),
            nn.InstanceNorm1d(time_dim, eps=1e-5, affine=True),
        )

        # Domain embeddings: let Mamba know which tokens are time vs freq
        self.domain_embed_time = nn.Parameter(torch.randn(1, 1, time_dim) * 0.02)
        self.domain_embed_freq = nn.Parameter(torch.randn(1, 1, time_dim) * 0.02)

        # Cross-Mamba: runs on concatenated [time; freq] sequence
        ...
        # Zero-initialized gate (identity at epoch 0)
    ...
    def forward(self, x_time, x_freq):
        ...
        Returns:
            ...
            x_freq_fused: (B, C_f, F_f, T_f) — unchanged (read-only)
        """
        ...
        # --- Prepare freq tokens ---
        freq_pooled = self.freq_pool_f(x_freq).squeeze(2)  # (B, C_f, T_f)
        freq_proj = self.freq_proj(freq_pooled)  # (B, D_t, T_f)
        freq_tokens = freq_proj.transpose(1, 2)  # (B, T_f, D_t)

        # --- Prepare time tokens ---
        time_tokens = x_time.transpose(1, 2)  # (B, L_t, D_t)

        # --- Add domain embeddings ---
        time_tokens_d = time_tokens + self.domain_embed_time
        freq_tokens_d = freq_tokens + self.domain_embed_freq

        # --- Concat and run Mamba ---
        concat_seq = torch.cat([time_tokens_d, freq_tokens_d], dim=1)  # (B, L_t+T_f, D_t)
        mamba_out = self.mamba(self.norm(concat_seq))  # (B, L_t+T_f, D_t)

        # --- Extract time portion only ---
        time_out = mamba_out[:, :L_t, :]  # (B, L_t, D_t)

        # --- Zero-init gated residual ---
        x_time_fused = (time_tokens + self.gate * time_out).transpose(1, 2)  # (B, D_t, L_t)
...
class DualDomainMambaEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        # ---- Cross-Mamba Fusion at each stage (eagerly initialized) ----
...
class DualDomainMambaCrossMamba(nn.Module):
