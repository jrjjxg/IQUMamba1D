"""DualDomainMambaSmall -- Compact variant of DualDomainMamba for IQ signal separation.

Targets 10-30M parameters (vs. 117M in the original DualDomainMamba) via:

  1. **Smaller freq_features_per_stage** (default [4, 8, 16, 32] vs. [16, 32, 64, 128])
     which directly controls freq-tower Mamba d_model = out_channels * freq_bins_out.
     This is the primary source of parameter reduction while fully preserving the
     original joint cross-frequency temporal modeling capability.

  2. **Wider time-domain channels** (e.g. [48, 96, 192, 384]) to compensate for
     the narrowed freq tower and better utilise the freed parameter budget.

  3. **Larger STFT hop_length** (default 128 vs. 64) to halve the number of
     STFT time frames, reducing cross-attention memory and compute.

  4. **Cross-attention dropout** (default 0.1) for regularisation, addressing
     the overfitting observed with larger models on small datasets.

The frequency-domain Mamba retains the original joint spectral modeling:
d_model = out_channels * freq_bins_out, so Mamba captures cross-frequency
temporal dynamics in one pass — identical to the original DualDomainMamba.
"""
...
class STFTFrontend(nn.Module):
    """Compute complex STFT and return (B, 4, F, T_stft) real tensor."""
...
# ============================================================================
#  Frequency-Domain Encoder Block (joint cross-frequency Mamba, same as original)
# ============================================================================

class FreqConvBlockSmall(nn.Module):
    """2-D Conv + joint cross-frequency Mamba along the STFT time axis.

    Modeling is identical to the original FreqConvBlock in DualDomainMamba:
        mamba_dim = out_channels * freq_bins_out
    so that Mamba captures cross-frequency temporal dynamics in one pass.

    Parameter reduction is achieved by using a smaller out_channels
    (controlled via freq_features_per_stage in the config) rather than
    changing the modeling architecture.
    """
    ...
    def __init__(self, in_channels: int, out_channels: int,
        ...
        # Joint cross-frequency Mamba: d_model = out_channels * freq_bins_out
        mamba_dim = out_channels * freq_bins_out
    ...
    def forward(self, x):
        ...
        h_mamba = self.mamba(h_flat)
        h_mamba = h_mamba.transpose(1, 2).reshape(B, C, Fr, T)

        return h + h_mamba


class FreqDomainEncoderSmall(nn.Module):
    """Multi-stage frequency-domain encoder using FreqConvBlockSmall."""

    def __init__(self, n_stages: int, features_per_stage: List[int],
                 input_length: int,
                 freq_features_per_stage: List[int] = None,
                 n_fft: int = 256, hop_length: int = 128, win_length: int = 256,
                 d_state: int = 16, d_conv: int = 4, expand: int = 2):
        super().__init__()
...
# ============================================================================
#  Dual-Domain Encoder (compact)
# ============================================================================

class DualDomainMambaEncoderSmall(nn.Module):
    """Time-domain Mamba encoder + lightweight freq tower + cross-attention."""
    ...
    def __init__(self,
        ...
        self.freq_encoder = FreqDomainEncoderSmall(
            n_stages=n_stages,
...
class DualDomainMambaSmall(nn.Module):
    """Compact Dual-Domain Mamba U-Net for IQ signal separation.

    Estimated parameter count with default config:
        features_per_stage=[48, 96, 192, 384],
        freq_features_per_stage=[16, 32, 64, 128],
        hop_length=128  -->  ~15-20M parameters
    """
    ...
    def __init__(self,
        ...
        super().__init__()
        self.encoder = DualDomainMambaEncoderSmall(
